### Silwood Park Mistletoe - Regressions and IPM
...
# Count occurrences
occs <- list()
for(i in 1:ncol(fig_s1_df)){
  occs[i] <- table(fig_s1_df[, i])[1]
}
mean(unlist(occs))
...
# Plot with geom_tile and export
Fig_S1 <- ggplot(fig_s1_df_long, aes(x = col, y = row, fill = fill_color)) +
...
# Calculate lambda empirically
pop_sizes <- colSums(intens_df[, 2:11])
emp_lambdas <- list()
for(i in 1:(length(pop_sizes) - 1)){
  emp_lambdas[i] <- pop_sizes[i+1] / pop_sizes[i]
}
emp_lambda <- mean(unlist(emp_lambdas))
# Empirical lambda of 1.08
...
# Plot and export Figure S2
Fig_S2 <- Fig_S2a + Fig_S2b + 
  plot_annotation(tag_levels = "a") &
  theme(plot.tag = element_text(size = 20))
Fig_S2
ggsave("Figure S2.png", Fig_S2)
...
# Create survival label
l_sur <- expression("Survival probability (" * italic("t") * " to " * italic("t") * "+1)")
...
sur_area_quad_glmm <- glmer(Survive ~ logArea_t0 + I(logArea_t0^2) + (1 | Indiv_ID), 
                       data = wrangled_df, family = binomial)
...
# Test model fit for chosen vital rate regression - Fig_S5a-b
par(mfrow = c(1, 2))
plot(residuals(sur_area_glmm) ~ fitted(sur_area_glmm), main = "Residuals vs Fitted values", xlab = "Fitted value", ylab = "Residuals")
...
# Plot berry production and export
Fig_S6 <- ggplot(data = wrangled_by_yr_df, aes(x = logArea, y = Berries)) +
...
# Assume constant survival as we cannot measure establishment s0, survival of 1-year-old seedlings s1 or survival of 2-year-old seedlings s2, s0 = s1 = s2
# therefore s0 = (Σrec/Σb)^(1/3)
# Σb is estimated using the berry production constant k, where b = k*exp(z)
# s0 will alternatively be estimated through iteration to obtain λ = 1.1
...
# Run IPM with establishment rate, 1yo and 2yo survival as constant
params <- data.frame(
...
yl_t1 <- expression(Area ~ (log ~ cm^2) ~ "in" ~ italic("t") * "+1")
...
plot.ipm <- function(P_kernel, F_kernel, ref_ht, mesh){
  ...
  final_plot <- plot_grid(main_plot, P_legend, F_legend, ncol = 3, rel_widths = c(3, 1, 0.5))
  return(final_plot)
}
...
write.csv(traits_sens_params_df, "Raw parameter sensitivity values.csv")
...
output_labels <- c("Lambda" = "Population growth rate", 
                   "R0" = "Net reproductive output", 
...
exp_vs_log_df <- data.frame(Berry_model = c(rep("Exponential", 6), rep("Logistic", 6)),
                            ...
                            Value = c(unlist(final_traits)[5:10], unlist(logistic_traits)[5:10]))
