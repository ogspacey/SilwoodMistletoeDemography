...
Fig_S1b <- ggplot(data = mst_hst_df, aes(x = Combined_height)) +
  geom_histogram() +
...
Fig_S3a <- ggplot(data = mst_hst_df, aes(x = Host_height, y = Combined_height)) +
  geom_point(aes(col = Genus)) +
  theme_bw() +
  geom_smooth(method = "lm", col = "darkgrey") +
  labs(x = "Host height (m)", y = "Mistletoe height (m)") +
  theme(legend.position = "none")
Fig_S3a

# Visualise relationship between host height and mistletoe intensity
Fig_S3b <- ggplot(data = intens_df, aes(x = Host_height, y = Max_I)) +
  geom_point(aes(col = Genus)) +
  theme_bw() +
  geom_smooth(method = "lm", col = "darkgrey") +
...
Fig_S1a <- ggplot(data = wrangled_by_yr_df, aes(x = logArea)) +
  geom_histogram() +
...
Fig_S2 <- ggplot(data = wrangled_by_yr_df, aes(x = logArea, y = Height)) +
  geom_point() +
  labs(x = xl, y = "Height above ground (m)") +
  theme_bw()
Fig_S2
...
for(i in 1:length(to_mods)){
  ...
  if(isSingular(to_mods[[i]]) == TRUE){
    vr_to_df[i,"Est_p"] <- "Singular model"
  }
...
write.csv(vr_to_df, "Table S3.csv")
...
Fig_S4 <- ggplot(data = wrangled_by_yr_df, aes(x = logArea, y = Berries)) +
...
Fig_S5a <- ggplot(data = rec_distrib, aes(x = x)) +
...
build.ipm <- function(params, mesh, ref_ht){
  ...
  # F_plot <- F/max(F)
  # 
  # # Combine sub-kernels into plottable kernel
  # K_plot <- P_plot + F_plot
  # 
  # Plot kernel
  # image(t(K_plot))
...
plot.ipm <- function(P_kernel, F_kernel, ref_ht, mesh){
  ...
  # Divide by maximum value for plotting
  plot_P <- t(ref_P/max(ref_P))
  plot_F <- t(ref_F/max(ref_F))
  ...
  main_plot <- ggplot() +
    ...
    geom_tile(data = dfP, aes(x = Var1, y = Var2, fill = value), alpha = 1) +
    ...
    scale_alpha_continuous(name = "Fecundity (scaled)", range = c(0, 1), 
                           guide = "none") +
    ...
    theme(panel.grid = element_blank(),
          ...
          axis.title.x = element_text(margin = margin(t = 5)),  # Moves x-axis label closer
          axis.title.y = element_text(margin = margin(r = 5)),  # Moves y-axis label closer
          
          # Ensure axis tick labels are close
  ...
  P_legend_plot <- ggplot(dfP, aes(x = Var1, y = Var2, fill = value)) +
    geom_tile() +
    scale_fill_gradient(name = "Transition probability", low = "white", high = colours(6)[6], limits = c(0, max(ref_P))) + 
    theme_void() +
    theme(legend.position = "right")  # Keep only legend
  
  #  Fake Legend Plot for F (Uses Fill to Simulate Alpha) 
  F_legend_plot <- ggplot(dfF, aes(x = Var1, y = Var2, fill = value)) +
    geom_tile() +
    scale_fill_gradient(name = "Fecundity", low = "white", high = colours(6)[3], limits = c(0, max(ref_F))) +  # Mimic alpha
    theme_void() +
    theme(legend.position = "right")  # Keep only legend
...
Fig_3 <- plot.ipm(P_kernel = outputs[["P"]], F_kernel = outputs[["F"]], ref_ht = mean_ht, mesh = outputs[["mesh"]])
...
Fig_S8d <- ggplot(data = traits_sens_mesh_df, aes(x = (mesh*mesh), y = Lmean)) +
  labs(x = "Total meshpoints", y = "Lmean") +
  geom_line() +
  theme_bw()
Fig_S8d
Fig_S8e <- ggplot(data = traits_sens_mesh_df, aes(x = (mesh*mesh), y = La)) +
  labs(x = "Total meshpoints", y = "La") +
  geom_line() +
  theme_bw()
Fig_S8e
Fig_S8f <- ggplot(data = traits_sens_mesh_df, aes(x = (mesh*mesh), y = Lamean)) +
  labs(x = "Total meshpoints", y = "Lamean") +
  geom_line() +
  theme_bw()
Fig_S8f

Fig_S8 <- (Fig_S8a + Fig_S8b) / (Fig_S8c + Fig_S8d) / (Fig_S8e + Fig_S8f) + plot_annotation(tag_levels = "a")
Fig_S8

# Sensitivity of traits to parameters (including k) ---------------------------------
traits_sens_params <- list()
traits_sens_params[[1]] <- traits
...
for(i in 2:(ncol(params)+1)){
  ...
  outputs <- build.ipm(params = final_params, mesh = 50, ref_ht = final_params$rec.mean.ht)
...
# Create data frame in which to store 
...
# Start with s0 from final model
...
# Sensitivity to functional form of berry production ----------------------
# Assume logistic relationship of berry production and size instead, with midpoint of half the largest size
...
Fig_S6a <- plot.ipm(P_kernel = outputs[["P"]], F_kernel = outputs[["F"]], ref_ht = hts[1], mesh = outputs[["mesh"]])
outputs <- build.ipm(params = params, mesh = 50, ref_ht = hts[2])
Fig_S6b <- plot.ipm(P_kernel = outputs[["P"]], F_kernel = outputs[["F"]], ref_ht = hts[2], mesh = outputs[["mesh"]])
outputs <- build.ipm(params = params, mesh = 50, ref_ht = hts[3])
Fig_S6c <- plot.ipm(P_kernel = outputs[["P"]], F_kernel = outputs[["F"]], ref_ht = hts[3], mesh = outputs[["mesh"]])
outputs <- build.ipm(params = params, mesh = 50, ref_ht = hts[4])
Fig_S6d <- plot.ipm(P_kernel = outputs[["P"]], F_kernel = outputs[["F"]], ref_ht = hts[4], mesh = outputs[["mesh"]])
outputs <- build.ipm(params = params, mesh = 50, ref_ht = hts[5])
Fig_S6e <- plot.ipm(P_kernel = outputs[["P"]], F_kernel = outputs[["F"]], ref_ht = hts[5], mesh = outputs[["mesh"]])